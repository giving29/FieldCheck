...
def main() -> int:
    ...
    # Recursive extractor — handles flat list, dict-of-lists, buckets-with-name
    # plus arbitrary nesting. An "athlete" is a leaf dict that has BOTH
    # `name` and `sport` AND no nested athletes/players/buckets list.
    ATHLETE_CHILD_KEYS = {'athletes', 'players', 'items', 'buckets', 'roster'}

    def looks_like_athlete(d: dict) -> bool:
        if not isinstance(d, dict):
            return False
        if 'name' not in d or 'sport' not in d:
            return False
        # Reject if it has an athlete-child container (it's a bucket then)
        for k in ATHLETE_CHILD_KEYS:
            if k in d and isinstance(d[k], list) and d[k] and isinstance(d[k][0], dict):
                return False
        return True

    def extract(obj, current_bucket=None) -> list:
        found = []
        if isinstance(obj, list):
            for item in obj:
                found.extend(extract(item, current_bucket))
        elif isinstance(obj, dict):
            if looks_like_athlete(obj):
                copy = dict(obj)
                if current_bucket and not copy.get('bucket'):
                    copy['bucket'] = current_bucket
                found.append(copy)
            else:
                # Container — use this node's name (if any) as the bucket label
                bucket_label = (obj.get('name') or obj.get('bucket')
                                or obj.get('category') or obj.get('tier') or current_bucket)
                for k, v in obj.items():
                    if isinstance(v, (list, dict)):
                        found.extend(extract(v, bucket_label))
        return found

    athletes = extract(roster)
    print(f"Found {len(athletes)} athletes in roster")
    if athletes:
        # Show bucket distribution for sanity
        from collections import Counter
        buckets = Counter(a.get('bucket') or '(no bucket)' for a in athletes)
        print("  bucket distribution:")
        for bk, ct in sorted(buckets.items(), key=lambda x: -x[1]):
